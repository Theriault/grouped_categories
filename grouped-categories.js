...
axisProto.setupGroups = function (options) {
  ...
  var d = userAttr && userAttr.length;
...
tickProto.render = function (index, old, opacity) {
  ...
  while (group = group.parent) {
    ...
    attrs = horiz ? {
      ...
      y: size + lvlSize / 2 + axis.groupFontHeights[depth] - bBox.height / 2 - 4 + userY / 2
    } : {
			x: size + lvlSize / 2 + userX,
			y: (minPos.y + maxPos.y - bBox.height) / 2  + axis.groupFontHeights[depth] + userY
		};
